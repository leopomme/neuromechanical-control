...
Alpha = [0.08 0.15 0.2]; %%%%%%%%%%%% TRY IN THE RANGE [0.08 0.2] 
Gamma = [0.001 0.04 0.1]; %%%%%%%%%%% TRY IN THE RANGE [0.001 0.04 0.1] 
...
for Trial=1:TrialStructure(end)%%%%%%%%%%%%%% TO BE EXTENDED %%%%%%%%%%%%
    ...
    if g==1
        uFF(:,i+1,Trial) = Alpha(j)*uPD(:,i,Trial)+(1-Gamma(1))*uFF(:,i,Trial);
    end
    if g==2
        uFF(:,i+1,Trial) = Alpha(1)*uPD(:,i,Trial)+(1-Gamma(j))*uFF(:,i,Trial);
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for Trial=1:TrialStructure(end)
    ...
    if j==1
        MaxDev1(Trial,1) = max(abs(x(1,:,Trial)));
    end
    if j==2
        MaxDev2(Trial,1) = max(abs(x(1,:,Trial)));
    end
    if j==3
        MaxDev3(Trial,1) = max(abs(x(1,:,Trial)));
    end
end
end
...
MaxY = 1.2*max(max(MaxDeviationData));
...
MaxYbis = 1.2*max(max(MaxDev3));

subplot(2,1,2); hold on;

plot(1:TrialStructure(end),MaxDev1,'--ro',1:TrialStructure(end),MaxDev2,'--go',1:TrialStructure(end),MaxDev3,'--bo','markersize',8,'linewidth',0.1)
hFill(1) = fill([1,1,TrialStructure(2)-1,TrialStructure(2)-1],[0,MaxYbis,MaxYbis,0],'b');
hFill(2) = fill([TrialStructure(2)-1,TrialStructure(2)-1,TrialStructure(3)-1,TrialStructure(3)-1],[0,MaxYbis,MaxYbis,0],'r');
hFill(3) = fill([TrialStructure(3)-1,TrialStructure(3)-1,TrialStructure(4)-1,TrialStructure(4)-1],[0,MaxYbis,MaxYbis,0],'g');
hFill(4) = fill([TrialStructure(4)-1,TrialStructure(4)-1,TrialStructure(5)-1,TrialStructure(5)-1],[0,MaxYbis,MaxYbis,0],'r');
hFill(5) = fill([TrialStructure(5)-1,TrialStructure(5)-1,TrialStructure(end),TrialStructure(end)],[0,MaxYbis,MaxYbis,0],'b');
...
legend('alpha = 0.08','alpha = 0.15','alpha = 0.2'); 
end
...
figure(5); close(5); f5=figure(5); set(f5,'name','2(b)','numbertitle','off'); set(gcf,'color','w');
...
legend('gamma = 0.08','gamma = 0.15','gamma = 0.2'); 
